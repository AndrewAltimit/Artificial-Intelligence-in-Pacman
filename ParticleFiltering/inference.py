...
class ExactInference(InferenceModule):
	...
	def elapseTime(self, gameState):
		...
		*** GORY DETAIL AHEAD ***
		...
		"*** YOUR CODE HERE ***"
		belief_update = util.Counter()
		
		# Previous Beliefs
		for pos, prob in self.beliefs.items():
			# Update using next ghost positions based on current game state
			for newPosDist in self.getPositionDistribution(self.setGhostPosition(gameState, pos)).items():
				belief_update[newPosDist[0]] += prob * newPosDist[1]

		# Beliefs must be normalized to represent a valid probability distribution
		belief_update.normalize()
		self.beliefs = belief_update
...
class ParticleFilter(InferenceModule):
	...
	def initializeUniformly(self, gameState):
		...
		"*** YOUR CODE HERE ***"
		self.particles = []
		maxval = len(self.legalPositions)
		for i in range(self.numParticles):
			index = i % maxval
			pos = self.legalPositions[index]
	...
	def observe(self, observation, gameState):
		...
		for pos in self.particles:
			...
			beliefs[pos] += emissionModel[distance]
		
		if sum(beliefs.values()) == 0:
			# All weights are zero, recreate from the prior distribution
			self.initializeUniformly(gameState)
			return
			
		# Create new list of particles based on sampling the current beliefs
		self.particles = [util.sample(beliefs) for particle in self.particles]
	...
	def elapseTime(self, gameState):
		...
		"""
		"*** YOUR CODE HERE ***"
		# For each particle, sample to get new position and update the list coordinates
		for i in range(len(self.particles)):
			newPos = util.sample(self.getPositionDistribution(self.setGhostPosition(gameState, self.particles[i])))
	...
	def getBeliefDistribution(self):
		...
		"""
		"*** YOUR CODE HERE ***"
		# Count each particle at every position
		beliefs = util.Counter()
		for pos in self.particles:
			beliefs[pos] += 1
