...
class GreedyBustersAgent(BustersAgent):
	...
	def chooseAction(self, gameState):
		...
		livingGhostPositionDistributions = [beliefs for i,beliefs
											...
											if livingGhosts[i+1]]
		"*** YOUR CODE HERE ***"
		# Add the ghost positions by using the position with the highest probability for each ghost
		ghosts = []
		for b in livingGhostPositionDistributions:
			ghosts.append(max(b, key=b.get))
			
		
		# For all legal actions, determine the distance to the ghost
		actions = []
		for a in legal:
			for g in ghosts:
				dist = self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, a), g)
				actions.append((dist, a))
				
		# Take the action that results from the min distance
